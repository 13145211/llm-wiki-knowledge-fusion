...
(function() {
    ...
    // ==================== 常量 ====================
    const STORE_PREFIX = 'dsup_';
    const DB_NAME = 'DeepSeekPdfUploader';
    const DB_VERSION = 1;
    const DEFAULT_INTERVAL_MINUTES = 10;
    const DEFAULT_MIN_WAIT_SECONDS = 120;
    const DEFAULT_STABLE_THRESHOLD = 12;       // DeepSeek 生成速度可能不同，稍放宽
    const DEFAULT_COOLDOWN_MINUTES = 120;
    const DEFAULT_WAKEUP_PROMPT = '你好，请帮我总结一下今天天气怎么样？';
    const DEFAULT_UPLOAD_WAIT_SECONDS = 8;    // DeepSeek 需要更长时间处理上传
    const DEFAULT_PROMPT_DONE_WAIT_SECONDS = 2;
    const DEFAULT_PRESEND_WAIT_SECONDS = 3;
    const FILE_INPUT_WAIT_TIMEOUT = 15000;
    const PANEL_ID = 'ds-uploader-panel';
    const DROP_OVERLAY_ID = 'ds-drop-overlay';
...
【XRD 衍射图】读出各衍射峰对应的 2θ 位置和晶面指数，用 Scherrer 方程算出的晶粒尺寸是否与文本一致？
【TEM/HRTEM 照片】目测估算颗粒尺寸范围（nm），与文本声称的对比，注意颗粒分布均匀性
【H₂-TPR 曲线】读出各还原峰的峰温度和相对面积，注意低温峰 vs 高温峰的归属
【XPS 谱图】核实结合能标注，峰的相对强度是否支持文本中的定性判断？
【活性曲线】直接读取关键数据点，验证文本中数字是否正确，检测失活趋势
【Arrhenius / TOF 图】读出表观活化能，验证文本中的 Eₐ 值
【Table 数据】扫描所有表格数据，验证文中引用的数字与表一致
...
图表深度读取指引：【XRD】记录各衍射峰 2θ/(hkl)，Scherrer 粒径 vs 文本值，检查杂峰
...
▪ Activity curves: Read key data points directly, verify claimed conversion values
▪ CO₂/H₂ curves: Track selectivity trends
            ...
            <div class="ds-content" id="ds-content">
                ...
                <div class="ds-section">
                    ...
                    <div class="ds-prompt-tabs" id="ds-prompt-tabs">
                        ...
                        <span class="ds-prompt-tab" data-tab="1">👨‍🏫 资深科学家</span>
